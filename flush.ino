...
constexpr int flush_ms = 10000; //You can use a smaller value like 4000 when testing

// Time at which to start sampling
// DateTime constructor takes  YYYY,MM, DD, HH, MM, SS
DateTime start_time = DateTime(2022, 4, 12, 18, 4, 00);
...
Sample samples[] = {
    Sample{.start=TimeSpan((int32_t)0,0,0,0), 15000UL}, //Sample 1 at T+0 seconds, ends at T+30 mins
    Sample{.start=TimeSpan(0,0,0,30), 15000UL}, //Sample 2 at T+30 minutes, ends at T+1 hr  there is a five minute buffer between
    Sample{.start=TimeSpan(0,0,1,0), 15000UL}, //Sample 3 at T+4 hr (3 hr gap from previous), ends at T+4.5
    Sample{.start=TimeSpan(0,0,1,30), 15000UL}, //Sample 4 at T+4.5 hr, ends at T+5
    Sample{.start=TimeSpan(0,0,2,0), 15000UL}, //Sample 5 at T+8 hr (3 hr gap from previous), ends at T+8.5
    Sample{.start=TimeSpan(0,0,2,30), 15000UL}, //Sample 6 at T+8.5 hr, ends at T+9
    Sample{.start=TimeSpan(0,0,3,0), 15000UL}, //Sample 7 at T+12 hr (3 hr gap from previous), ends at T+12.5
    Sample{.start=TimeSpan(0,0,3,30), 15000UL}, //Sample 8 at T+12.5 hr, ends at T+13 hr
    Sample{.start=TimeSpan(0,0,4,0), 15000UL}, //Sample 9 at T+16 hr (3 hr gap from previous), ends at T+16.5
    Sample{.start=TimeSpan(0,0,4,30), 15000UL}, //Sample 10 at T+16.5 hr, ends at T+17
    Sample{.start=TimeSpan(0,0,5,0), 15000UL}, //Sample 11 at T+20 hr (3 hr gap from previous), ends at T+20.5
    Sample{.start=TimeSpan(0,0,5,30), 15000UL}, //Sample 12 at T+20.5 hr, ends at T+21
};

// If you wish to sample periodically i.e. at regular intervals set this to the
// period of the cycle, otherwise, ignore it.
TimeSpan period = TimeSpan(0,0,2,0);
...
constexpr unsigned num_periods = 1;
